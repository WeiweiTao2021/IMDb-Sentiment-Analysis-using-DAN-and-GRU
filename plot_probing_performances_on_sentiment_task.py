...
if __name__ == '__main__':
    ...
    for (ax, seq2vec_name, layer_range) in zip(axs.flat, choices.keys(), choices.values()):
        ax.plot(layer_range, probing_accuracies[seq2vec_name])
        ax.set_xlabel("Probing Layer")
        ax.set_ylabel("Accuracy")
        title = f"{seq2vec_name.upper()}: Probing Performance vs Probing Layer"
        ax.set_title(title)

    fig.subplots_adjust(hspace = 0.5)
    plt.savefig(os.path.join("plots", f"probing_performance_on_sentiment_task.png"))
    plt.clf()
