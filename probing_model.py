...
## python3 train.py probing  data/imdb_sentiment_train_5k.jsonl data/imdb_sentiment_dev.jsonl --base-model-dir serialization_dirs/main_dan_5k_with_emb --num-epochs 5 --layer-num 3        
## python3 train.py probing  data/imdb_sentiment_train_5k.jsonl data/imdb_sentiment_dev.jsonl --base-model-dir serialization_dirs/main_gru_5k_with_emb --num-epochs 5 --layer-num 1
class ProbingClassifier(nn.Module):
    def __init__(self,
        ...
        # TODO(students): start
        self._classification_layer = nn.Linear(input_dim, classes_num).to(device)
        # TODO(students): end
    ...
    def forward(self, inputs: torch.Tensor, training: bool = False) -> torch.Tensor:
        ...
        #print("size of model layers are ", self._pretrained_model(inputs)['layer_representations'].shape)
        last_layer = self._pretrained_model(inputs)['layer_representations'][self._layer_num-1]
        logits = self._classification_layer(last_layer)
        # TODO(students): end
