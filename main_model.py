...
class MainClassifier(nn.Module):
    def __init__(self,
        ...
        # initiate the embeddings: it is actually load from glove embedding
        self._embedding_layer = nn.Embedding(vocab_size, embedding_dim).to(device)

        # classification laryer is to generate logits
    ...
    def forward(self,
        ...
        """
        # input text: ids of tokens
        inputs = inputs.to(self.device)

        # generate embedding of the each batch words
        embedded_tokens = self._embedding_layer(inputs)

        # 0 is standing for padding token; this is standing for whether it is a padding token or not
        tokens_mask = torch.where(inputs !=0, 1,0).float()
        
        ## generate outputs using DAN or GRU
        ...
        # calssification vector will be used to generate logits
