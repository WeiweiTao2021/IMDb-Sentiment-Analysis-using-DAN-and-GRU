...
# spacy to load the english model
...
def read_instances(data_file_path: str,
    ...
    with open(data_file_path) as file:
        for line in tqdm(file.readlines()):
            ...
            text = instance["text"]
            ## find all tokens for each line of text
            ...
            instance.pop("text")
            instances.append(instance) ## return labels and text tokens
    return instances
...
def build_vocabulary(instances: List[Dict],
    ...
    # make sure pad_token is on index 0 and unknown token is 1
    UNK_TOKEN = "@UNK@"
    PAD_TOKEN = "@PAD@"
    token_to_id = {PAD_TOKEN: 0, UNK_TOKEN: 1}

    # First add tokens which were explicitly passed. add common words in glove
    ...
    # words contains all tokens of all lines of text
...
def index_instances(instances: List[Dict], token_to_id: Dict) -> List[Dict]:
    ...
    for instance in instances:
        ...
        instance.pop("text_tokens") #replace tokens using token ids
    return instances
